...
import { SortableProcessStepItem } from '@/components/admin/SortableProcessStepItem';
...
export default function ProcessEditorPage() {
  ...
  const handleSaveModal = () => {
    ...
    if (editingIndex !== null) {
      ...
      updatedSteps[editingIndex] = {
        step: editingIndex + 1, // Step number is based on position
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await homeService.updateProcess(payload.process);

      if (response.success) {
        alert('Section Processus mise à jour avec succès !');
      ...
      <div className="bg-white border-b border-neutral-200">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="mt-2 text-neutral-600">
                  Gérez les étapes du processus affichées sur votre page d'accueil
                </p>
              </div>
            ...
            {/* Publish Toggle */}
            <PublishToggle section="process" initialPublished={false} />
          </motion.div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          >
            ...
            {/* Process Steps List with drag-and-drop */}
            {processSteps.length > 0 ? (
              <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleStepDragEnd}>
                <SortableContext items={processSteps.map((_, i) => String(i))} strategy={verticalListSortingStrategy}>
            ...
            {processSteps.length > 0 && (
              <p className="text-xs text-neutral-500 mt-4">
                ...
                Les numéros d'étape sont automatiquement générés en fonction de l'ordre
              </p>
            )}
          </motion.div>
          ...
          >
                ...
                <div className="space-y-8">
                  {processSteps.map((step, index) => (
                    <div key={index} className="flex items-start gap-6">
                      {/* Step Number */}
                      <div className="flex-shrink-0 w-16 h-16 bg-amber-600 text-white rounded-full flex items-center justify-center font-bold text-2xl shadow-lg">
                        {step.step}
                      </div>
                      ...
                      <div className="flex-1">
                        ...
                        {/* Image */}
                        {step.image && (
                          <div className="relative aspect-video w-full max-w-md bg-neutral-100 rounded-lg overflow-hidden shadow-sm">
                            <Image
                              src={step.image}
                              alt={step.title}
                              fill
            ...
            <p className="text-xs text-neutral-500 mt-3 text-center">
              ...
              Ceci est un aperçu de la section Processus telle qu'elle apparaîtra sur votre site
            </p>
          </motion.div>
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
                    <div className="flex items-start">
                      ...
                      <div>
                        <h4 className="text-sm font-medium text-amber-800">Numéro d'étape automatique</h4>
                        <p className="mt-1 text-sm text-amber-700">
                          Le numéro d'étape sera automatiquement généré en fonction de la position dans la liste
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={4}
                      placeholder="Ex: Nous discutons de vos besoins, de votre budget et de vos préférences de style"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-colors ${
                        modalErrors.image ? 'border-red-500' : 'border-neutral-300'
                      }`}
                    />
                    {modalErrors.image && (
                      <p className="text-sm text-red-600 mt-1">{modalErrors.image}</p>
                    )}
